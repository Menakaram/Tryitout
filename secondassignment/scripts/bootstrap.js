...
(function () {
  ...
  function logRejectedPromises() {
    unhandledRejections.forEach(function (reason) {
      LITTLEQAIROES-PROJECT.Analytics.trackError('UnhandledPromiseRejection', reason);
    });
  ...
  function lazyLoadWCPolyfillsIfNecessary(callback) {
    ...
    if (!(LITTLEQAIROES-PROJECT.Util.getChromeVersion() &&
      LITTLEQAIROES-PROJECT.Util.getChromeVersion() >= 46 || LITTLEQAIROES-PROJECT.Util.getFirefoxVersion() && LITTLEQAIROES-PROJECT.Util.getFirefoxVersion() >= 40)) {
      var parent = document.querySelector('body');
  ...
  window.addEventListener('offline', function () {
    if (LITTLEQAIROES-PROJECT.Elements && LITTLEQAIROES-PROJECT.Elements.Template.$.toast) {
      LITTLEQAIROES-PROJECT.Elements.Template.$.toast.show(
  ...
  window.addEventListener('beforeinstallprompt', function (event) {
    LITTLEQAIROES-PROJECT.Analytics.trackEvent('installprompt', 'fired');

    event.userChoice.then(function (choiceResult) {
      // choiceResult.outcome will be 'accepted' or 'dismissed'.
      // choiceResult.platform will be 'web' or 'android' if the prompt was
      // accepted, or '' if the prompt was dismissed.
      LITTLEQAIROES-PROJECT.Analytics.trackEvent('installprompt', choiceResult.outcome,
  ...
  function initApp() {
    ...
    lazyLoadWCPolyfillsIfNecessary(function () {
      LITTLEQAIROES-PROJECT.Elements.init();
    });
  }
